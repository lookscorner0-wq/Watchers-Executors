...
EVOLUTION_URL        = os.environ.get("EVOLUTION_URL", "")          # e.g. https://p01--whatsapp--xxx.code.run
EVOLUTION_API_KEY    = os.environ.get("EVOLUTION_API_KEY", "")      # global API key
EVOLUTION_INSTANCE   = os.environ.get("EVOLUTION_INSTANCE", "whatapp")  # instance name
...
# ── Startup message flag (only once per server run) ─────────
_STARTUP_MSG_SENT = False
_STARTUP_MSG_LOCK = threading.Lock()
...
# ============================================================
# AUTO DATABASE SETUP  (LangGraph checkpoints + agency tables)
# ============================================================
...
def wpp_send(number: str, message: str, typing: bool = True) -> bool:
    ...
    try:
        ...
        resp.raise_for_status()
        data = resp.json()
        if data.get("key") or data.get("status") == "PENDING" or resp.status_code == 201:
            return True
        log.warning(f"[Evolution] Send response: {data}")
        return True  # evolution returns 200/201 on success
    except Exception as exc:
...
def gmail_reply(to: str, subject: str, original_body: str, reply_text: str) -> bool:
    reply = call_llm([{
        "role": "user",
        "content": (
            f"Email from: {to}\nSubject: {subject}\n"
...
def playwright_scrape_linkedin(query: str) -> list[dict]:
    ...
    try:
        with sync_playwright() as pw:
            ...
            page = ctx.new_page()
            page.goto(
                f"https://www.linkedin.com/search/results/content/?keywords={query.replace(' ','%20')}&sortBy=date_posted",
                timeout=25000,
            )
...
def manager_handle_agent_msg_node(state: ManagerState) -> ManagerState:
    ...
    if msg_type == "oos_decision_request":
        ...
        wpp_send(CEO_WHATSAPP, f"📊 [{AGENCY_NAME}] Daily Research:\n\n{summary[:1000]}")
...
def researcher_daily_cycle():
    log.info(f"[{AGENCY_NAME} Researcher] Daily cycle start")
    ...
    for f in as_completed(futures):
        exc = f.exception()
        if exc:
            log.error(f"[{AGENCY_NAME} Researcher] Future error: {exc}")
...
# ============================================================
# STARTUP MESSAGE — Only once per server start
# ============================================================

def send_startup_message():
    """
    Sent ONCE when server starts.
    Waits for Evolution API to be connected, then sends.
    After this, manager only messages CEO when there's actual news.
    """
    global _STARTUP_MSG_SENT

    with _STARTUP_MSG_LOCK:
        if _STARTUP_MSG_SENT:
            return
...
def main():
    ...
    # Auto-create all DB tables (LangGraph + agency tables)
    setup_database()

    # Send startup WhatsApp message to CEO (once only)
    threading.Thread(target=send_startup_message, daemon=True,
                     name="startup-msg").start()

    # NOTIFY listener — event-driven
    threading.Thread(target=core_notify_listener, daemon=True,
                     name="notify-listener").start()

    # Manager polling fallback (WhatsApp replies, Gmail)
